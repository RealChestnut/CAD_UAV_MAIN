...
void battery_add_Callback(const std_msgs::Int16& msg);
...
double mass_main = 8.1; //9.0; // main drone mass(kg) 
double mass_sub1 = 9.320; // sub1 drone mass (kg)
...
ros::Subscriber battery_checker_add;
...
ros::Publisher battery_voltage_add;
...
double x_y = 0;
double x_z = 0;
double lin_vel_cut_off_freq = 0.005;
...
std_msgs::Float32 battery_voltage_add_msg;
...
void shape_detector()
{
  ...
  if(/*(button_cnt==(50*button_limit)) &&*/ mono_flight ){ // we define that this state is disassembled
   ...
   //if(DOB_mode){ROS_INFO("DOB_mode_ON!!!!!!!!!!!!!!!!!");};
...
if(mono_flight){
	...
	Par = 20.0; //combined :: 15
        Iar = 0.0; //combined :: 0
        Dar = 20.0; //combined :: 7

        Pap = 20.0; //combined :: 15
        Iap = 0; //combined :: 0
        Dap = 14.0; //combined :: 7
...
void UpdateParameter(int num)
{
  ...
  if(num==1){
  		...
  		if(CoM_update_mode==false){	  
	  	CoM_hat.x = -0.004;
                CoM_hat.y = -0.021;//-0.025;
                CoM_hat.z = -0.065;
		x_c_y = 0;
		y_c_y = 0;
		//ROS_INFO("COM UPDATE NOT");
		}
		else{
			//ROS_INFO("COM UPDATE");
		...
		x_c_y =0; //나중에 수정
		CoM_hat_log.x = -0.004 + x_c_y; 
		// + x_c_y;
                CoM_hat_log.y = -0.021 + y_c_y;
		//-0.025;
                CoM_hat_log.z = -0.065;
...
void Command_Generator()
{
  ...
  if(true/*sbus_mode*/)
  {
    ...
    if(main_agent){/// 메인드론인 경우 Sbus[0]를 yaw desired input으로 사용 yaw desired input으로 사용, 서브드론은 sbus[3]
    ...
    XYZ_desired.y = XYZ_desired_base.y + XY_limit*(((double)Sbus[3]-(double)1500)/(double)500); //for sub drone :: Sbus[0]
    ...
    if(altitude_mode){ 
      ...
      if(XYZ_desired.z <-altitude_limit) XYZ_desired.z=-altitude_limit;
  ...
  double deltaPx = (main_position_opti_new.x-sub_position_opti.x);
  double deltaPy = (main_position_opti_new.y-sub_position_opti.y);
  double deltaPz = (main_position_opti_new.z-sub_position_opti.z);
  ...
  if(is_Appr)
  {
   ...
   /* 
    ...
    rpy_desired.y = 0.0;
    rpy_desired.z = 0.0;
    // X position의 경우 자기 위치에서 deltaP(main_posi-sub_posi)만큼 추가로 이동 하는 명령
    // delta P가 abs(P)=0.005 이라면 자기 위치 고수
    XYZ_desired.x = XYZ_desired_base.x + deltaPx;
    if(deltaPx <= 0.005) XYZ_desired.x = sub_position_opti.x;
    if(deltaPx > -0.005) XYZ_desired.x = sub_position_opti.x;
    
    // Y position의 경우 자기 위치에서 deltaP(main_posi-sub_posi)만큼 추가로 이동하고 +10cm간격 유지하는 명령
    // deltaP가 abs(P)<=0.1이라면 자기 위치 고수
    XYZ_desired.y = XYZ_desired_base.y + deltaPy;
    if(deltaPy<=0.11) XYZ_desired.y = sub_position_opti.y;
    if(deltaPy>0.09) XYZ_desired.y = sub_position_opti.y;
    if(deltaPy<=0.09) XYZ_desired.y = sub_position_opti.y-(0.1-deltaPy);


    XYZ_desired.z =  sub_position_opti.z + deltaPz;
  ...
  if(is_Dock){
    ...
    // Y position의 경우 자기 위치에서 deltaP(main_posi-sub_posi)만큼 추가로 이동하고 +10cm간격 유지하는 명령
    // deltaP가 abs(P)<=0.1이라면 자기 위치 고수
    XYZ_desired.y = XYZ_desired_base.y + deltaPy; //docking 하면 계속 돌진
    if(deltaPy<=0.001) XYZ_desired.y = sub_position_opti.y-(0.1-deltaPy); //혹시 결합 안될때를 대비함
    XYZ_desired.z =  sub_position_opti.z + deltaPz;
...
void reset_data()
{
  ...
  XYZ_desired_base.z=position_from_t265.z;//main_position_opti_new.z;
...
void PublishData()
{
  ...
  Center_of_Mass.publish(CoM_hat_log); // CoM
  Force_allocation_factor.publish(alpha_data);
  battery_voltage.publish(battery_voltage_msg);
  battery_voltage_add.publish(battery_voltage_add_msg);
...
void imu_Callback(const sensor_msgs::Imu& msg)
{
    ...
    tf::Matrix3x3(quat).getRPY(imu_rpy.x,imu_rpy.y,imu_rpy.z); //t265_rpy.z
...
void sbus_Callback(const std_msgs::Int16MultiArray::ConstPtr& array)
{
  ...
  //if(main_agent && mono_flight){ if sub drone
      ...
      if(Sbus[6]<1300){
        ...
        velocity_mode=false;
        position_mode=false;
	CoM_update_mode = false;
      }
      else if(Sbus[6]<1700){
        attitude_mode=false;
        velocity_mode=false;
        position_mode=true;
	CoM_update_mode = true;
...
double voltage_add = 0.0;
double voltage_old_add = 0.0;
//BATTERY DATA CALLBACK//
void battery_add_Callback(const std_msgs::Int16& msg)
{
    
    int16_t value=msg.data;
    voltage_add=value*5.0/(double)1024/(7440./(30000.+7440.)); //4096
    double kv=0.08;
    voltage_add=kv*voltage_add+(1-kv)*voltage_old_add;
    voltage_old_add=voltage_add;
    if(voltage_add>25.2) voltage_add=25.2;
    if(voltage_add<20.0) voltage_add=20.0;
    battery_voltage_add_msg.data=voltage_add;
