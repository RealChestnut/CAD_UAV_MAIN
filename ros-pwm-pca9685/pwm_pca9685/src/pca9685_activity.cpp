...
PCA9685Activity::PCA9685Activity(ros::NodeHandle &_nh, ros::NodeHandle &_nh_priv) :
  nh(_nh), nh_priv(_nh_priv) {
    ...
    nh_priv.param("device", param_device, (std::string)"/dev/i2c-2");
    nh_priv.param("address", param_address, 0x40);
    nh_priv.param("frequency", param_frequency, (int)417);
...
bool PCA9685Activity::reset() {
    ...
    _i2c_smbus_write_byte_data(file, PCA9685_PRESCALE_REG, 14); // set prescale
    ros::Duration(0.025).sleep();
